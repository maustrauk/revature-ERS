...
public class ReimbursementDaoImpl implements ReimbursementDao{
	...
	public List<Reimbursement> getAll() {
		...
		try(Connection con = dbCon.getDBConnection()) {
			...
			while(result.next()) 
			{
				reimbursementsList.add(new Reimbursement(result.getInt(1), result.getDouble(2), result.getString(3), result.getString(4), result.getString(5), result.getString(6), result.getInt(7), result.getInt(8), result.getInt(9), result.getInt(10)));
			}
	...
	public Reimbursement getById(int id) {
		...
		try(Connection con = dbCon.getDBConnection()) {
			...
			while(result.next()) {
				reimbursement = new Reimbursement(result.getInt(1), result.getDouble(2), result.getString(3), result.getString(4), result.getString(5), result.getString(6), result.getInt(7), result.getInt(8), result.getInt(9), result.getInt(10));
			}
		} catch (SQLException e) {
			e.fillInStackTrace();
		}
	...
	public Reimbursement update(Reimbursement entity) {
		...
		try(Connection con = dbCon.getDBConnection()) {
			...
			statement.setString(7, entity.getReimbReceipt());
	...
	public Reimbursement insert(Reimbursement entity) {
		try(Connection con = dbCon.getDBConnection()) {
			...
			statement.setString(6, entity.getReimbReceipt());
